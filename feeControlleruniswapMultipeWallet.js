...
async function checkAllLPPositions() {
    try {
        ...
        const YOUR_WALLET = ['0x1532982cDC00bd86d4422136Caa444CC068Fc8ef', '0x9Cd915dce319818487E909bcbC7eDA6dcCEc4b5E'];
        ...
        for (let i = 0; i < pairCount; i++) {
            try {
                ...
                // Get LP balance for all wallets
                console.log('Checking LP balance for all wallets...');
                let lpBalance = 0n;
                for (const wallet of YOUR_WALLET) {
                    console.log(`Checking wallet: ${wallet}`);
                    const balance = await pairContract.balanceOf(wallet);
                    console.log(`Balance for ${wallet}: ${balance}`);
                    lpBalance += balance;
                }
                console.log(`Total LP Balance across all wallets: ${lpBalance}`);

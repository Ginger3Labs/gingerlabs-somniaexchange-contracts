...
if (!MONGODB_DB_NAME || !RPC_URL || !TARGET_TOKEN_ADDRESS || !FACTORY_ADDRESS || !ROUTER_ADDRESS || !WRAPPED_TOKEN_ADDRESS) {
    throw new Error('One or more environment variables are not set for bulk position update.');
}
...
async function calculateAndUpdatePosition(pairAddress: string, walletAddress: string): Promise<AnyBulkWriteOperation | null> {
    try {
        ...
        return {
            updateOne: {
                filter: { walletAddress: getAddress(walletAddress), pairAddress: getAddress(pairAddress) },
                update: { $set: updatedPosition },
                upsert: true
            }
        };
    } catch (error) {
        console.error(`Failed to process pair ${pairAddress} for wallet ${walletAddress}:`, error);
        return null; // Skip this pair on error
    }
}
...
export async function POST(request: NextRequest) {
    try {
        const { pairAddresses, walletAddress } = await request.json();

        if (!Array.isArray(pairAddresses) || pairAddresses.length === 0 || !walletAddress || !ethers.isAddress(walletAddress)) {
            return NextResponse.json({ success: false, message: 'A valid array of pairAddresses and a walletAddress are required.' }, { status: 400 });
        }
        ...
        const updatePromises = pairAddresses.map(pairAddress =>
            calculateAndUpdatePosition(pairAddress, walletAddress)
        );

        const bulkOperations = (await Promise.all(updatePromises)).filter(op => op !== null) as AnyBulkWriteOperation[];

        if (bulkOperations.length > 0) {
            await positionsCollection.bulkWrite(bulkOperations);
            console.log(`Successfully processed ${bulkOperations.length} positions for wallet ${walletAddress}.`);
        } else {
            console.log(`No valid positions to update for wallet ${walletAddress}.`);
        }

        return NextResponse.json({ success: true, message: `Processed ${bulkOperations.length} positions.` });
